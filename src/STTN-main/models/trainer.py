...
def model_train(inputs, blocks, args, sum_path='./output/tensorboard'):
    ...
    with tf.Session() as sess:
        writer = tf.summary.FileWriter(r'./output/tensorboard/train', sess.graph)
        ...
        for i in range(epoch):
            ...
            for j, x_batch in enumerate(
                ...
                summary, _ = sess.run([merged, train_op], feed_dict={
                    x: x_batch[0][:, 0:n_his + 1, :, :], 
                    keep_prob: 0.8,
                    is_training: True
                })
                writer.add_summary(summary, i * epoch_step + j)
                if j % 50 == 0:
                    loss_value = sess.run([train_loss_w, train_loss, copy_loss], feed_dict={
                    x: x_batch[0][:, 0:n_his + 1, :, :], 
                    keep_prob: 0.8,
                    is_training: True
                })
